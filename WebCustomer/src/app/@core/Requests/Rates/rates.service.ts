...
export class RatesService {
  ...
  private GetRate = this.urlService + '/api/Rate/PriceTransport';
  constructor(private http: HttpClient) {}

  GetPackageRate(cityCode: string): Observable<Array<PackageInfo>> {
    const httpOptions = {
    ...
    return (
      this.http
        .post(this.GetRate, JSON.stringify(this.SetGetRateValueJson(cityCode)), httpOptions)
        .map((res) => {
  ...
  private SetGetRateValueJson(countryCode: string) {
    const x = {
      city: countryCode,
    };
    return x;
